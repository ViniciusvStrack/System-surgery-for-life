...
const bot = new StoreBot({ catalog, sessions, orders, commerce, faqFile: path.join(config.root, "data/faqs.json"), config });
const whatsapp = new WhatsAppClient(config);
const authRateLimiter = new SlidingWindowRateLimiter({ limit: 30, windowMs: 60_000 });
const orderRateLimiter = new SlidingWindowRateLimiter({ limit: 12, windowMs: 15 * 60_000 });
...
const googleOauthConfigured = Boolean(config.googleOauthClientId && config.googleOauthClientSecret && config.googleOauthCallbackUrl);
const customerAuth = googleOauthConfigured ? new CustomerAuthService({
  ...
  transactionsFile: path.join(config.root, "runtime/customer-oauth-transactions.json"),
...
const publicFiles = {
  ...
  "/privacidade": ["loja/privacidade.html", "text/html; charset=utf-8", "store"],
  ...
  "/assets/brand/sfl-logo-stacked.png": ["assets/brand/sfl-logo-stacked.png", "image/png", "asset"],
  "/assets/brand/sfl-logo-horizontal.png": ["assets/brand/sfl-logo-horizontal.png", "image/png", "asset"],
  "/assets/brand/sfl-logo-monogram.png": ["assets/brand/sfl-logo-monogram.png", "image/png", "asset"],
  "/assets/brand/sfl-monogram-512.png": ["assets/brand/sfl-monogram-512.png", "image/png", "asset"],
  "/simulador": ["index.html", "text/html; charset=utf-8", "simulator"],
  "/simulador/": ["index.html", "text/html; charset=utf-8", "simulator"],
  "/simulador/styles.css": ["styles.css", "text/css; charset=utf-8", "simulator"],
  "/simulador/app.js": ["app.js", "text/javascript; charset=utf-8", "simulator"],
...
function sendJson(response, status, value, headers = {}) {
  response.writeHead(status, { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store", "X-Content-Type-Options": "nosniff", "Referrer-Policy": "no-referrer", "X-Frame-Options": "DENY", ...(status === 413 ? { Connection: "close" } : {}), ...headers });
  response.end(JSON.stringify(value));
}

function redirect(response, status, location, headers = {}) {
  response.writeHead(status, { Location: location, "Cache-Control": "no-store", "X-Content-Type-Options": "nosniff", "Referrer-Policy": "no-referrer", ...headers });
  response.end();
}
...
function sendStaticFile(response, absolutePath, type, cache) {
  ...
  stream.once("open", () => {
    ...
    pipeline(stream, response, (error) => { if (error && !response.destroyed) response.destroy(); });
  });
  stream.once("error", () => {
    if (!opened && !response.headersSent) sendJson(response, 404, { error: "Não encontrado" });
...
function sendStoreError(response, error) {
  ...
  const message = status >= 500
    ? (code === "STORE_NOT_READY" ? "A loja ainda não está pronta para receber pedidos." : "Não foi possível processar o pedido agora.")
    : String(error?.message || "Pedido inválido.").slice(0, 200);
  const details = status === 409 && error?.details && typeof error.details === "object" ? error.details : undefined;
  return sendJson(response, status, { error: { code, message, ...(details ? { details } : {}) } });
}
  ...
  return new Promise((resolve, reject) => {
    ...
    const onData = (chunk) => {
      ...
      if (size > maxBytes) {
        settled = true;
        request.pause();
        reject(Object.assign(new Error("Payload excede o limite permitido."), { status: 413 }));
        return;
      }
      chunks.push(chunk);
    };
...
async function handleRequest(request, response) {
  ...
  catch { return sendJson(response, 400, { error: "Solicitação inválida." }); }
  ...
  if (request.method === "POST" && url.pathname === "/api/auth/forgot-password") { const body = JSON.parse((await readBody(request)).toString("utf8")); const token = auth.requestReset(body.email); return sendJson(response, 200, { message: "Se a conta existir, as instruções serão enviadas.", developmentResetToken: config.exposeDevelopmentResetToken ? token : undefined }); }
  ...
  if (url.pathname === "/api/inventory/snapshot") {
    ...
    if (request.method === "GET") {
      try { return sendJson(response, 200, inventory.snapshot()); }
      catch (error) { return sendJson(response, error.status || 503, { error: "O estoque está temporariamente indisponível.", code: error.code || "INVENTORY_UNAVAILABLE" }); }
    }
    if (request.method === "PUT") {
      try {
        ...
        return sendJson(response, error.status || (error.code === "REVISION_CONFLICT" ? 409 : 400), { error: error.status >= 500 ? "O estoque está temporariamente indisponível." : error.message, code: error.code, currentRevision: error.currentRevision });
      }
    }
  }
  if (request.method === "GET" && url.pathname === "/api/catalog") {
    try { return sendJson(response, 200, catalog.available()); }
    catch (error) { return sendStoreError(response, error); }
  }
  if (request.method === "GET" && publicFiles[url.pathname]) {
    const [file, type, scope] = publicFiles[url.pathname];
    if (scope === "simulator" && !config.simulatorEnabled) return sendJson(response, 404, { error: "Não encontrado" });
    const cache = scope === "asset" ? "public, max-age=86400" : "no-store";
    return sendStaticFile(response, path.join(config.root, "public", file), type, cache);
  }
  if (request.method === "GET" && url.pathname === "/api/customer-auth/google/start") {
    if (!customerAuth) return sendJson(response, 503, { error: "Acesso com Google temporariamente indisponível." });
    const rate = authRateLimiter.consume(`google:${request.socket.remoteAddress || "unknown"}`);
    if (!rate.allowed) return sendJson(response, 429, { error: "Muitas solicitações. Tente novamente em instantes." }, { "Retry-After": String(rate.retryAfterSeconds) });
    try {
      const result = customerAuth.beginLogin({ returnTo: url.searchParams.get("returnTo") || "/loja" });
      return redirect(response, 302, result.authorizationUrl, { "Set-Cookie": result.cookie });
    } catch (error) { return sendJson(response, error.status || 400, { error: "Não foi possível iniciar o acesso com Google." }); }
  }
  if (request.method === "GET" && url.pathname === "/api/customer-auth/google/callback") {
    if (!customerAuth) return redirect(response, 303, "/loja?login=erro");
    const rate = authRateLimiter.consume(`google:${request.socket.remoteAddress || "unknown"}`);
    if (!rate.allowed) return redirect(response, 303, "/loja?login=erro", { "Retry-After": String(rate.retryAfterSeconds), "Set-Cookie": customerAuth.clearTransientCookie() });
    let returnTo = "/loja";
    try {
      if (url.searchParams.has("error")) {
        const transaction = customerAuth.consumeTransaction(request.headers.cookie || "", url.searchParams.get("state"));
        returnTo = transaction.returnTo;
        return redirect(response, 303, loginErrorLocation(returnTo), { "Set-Cookie": customerAuth.clearTransientCookie() });
      }
      const result = await customerAuth.completeLogin({ code: url.searchParams.get("code"), state: url.searchParams.get("state"), cookieHeader: request.headers.cookie || "" }, requestContext(request));
      return redirect(response, 303, result.returnTo, { "Set-Cookie": result.setCookies });
    } catch {
      return redirect(response, 303, loginErrorLocation(returnTo), { "Set-Cookie": customerAuth.clearTransientCookie() });
    }
  }
  if (request.method === "GET" && url.pathname === "/api/customer-auth/me") {
    if (!customerAuth) return sendJson(response, 503, { error: "Acesso com Google temporariamente indisponível." });
    try { return sendJson(response, 200, customerAuth.me(request)); }
    catch (error) { return sendJson(response, error.status || 401, { error: "Autenticação necessária." }); }
  }
  if (request.method === "POST" && url.pathname === "/api/customer-auth/logout") {
    if (!customerAuth) return sendJson(response, 503, { error: "Acesso com Google temporariamente indisponível." });
    try { const result = customerAuth.logout(request); return sendJson(response, 200, { ok: true }, { "Set-Cookie": result.cookie }); }
    catch (error) { return sendJson(response, error.status || 400, { error: error.status === 403 ? "Proteção CSRF inválida." : "Autenticação necessária." }); }
  }
  ...
  if (request.method === "POST" && url.pathname === "/api/store/orders") {
    try {
      if (!String(request.headers["content-type"] || "").toLowerCase().startsWith("application/json")) {
        throw Object.assign(new Error("Content-Type deve ser application/json."), { status: 400, code: "INVALID_ORDER" });
      }
      const principal = orderPrincipal(request);
      const rate = orderRateLimiter.consume(principal);
      if (!rate.allowed) {
        return sendJson(response, 429, { error: { code: "ORDER_RATE_LIMITED", message: "Muitas tentativas de pedido. Aguarde alguns minutos." } }, { "Retry-After": String(rate.retryAfterSeconds) });
      }
      const payload = JSON.parse((await readBody(request, 32_768)).toString("utf8"));
...
const server = http.createServer((request, response) => {
  void handleRequest(request, response).catch((error) => {
    console.error("Falha não tratada na requisição", { name: error?.name || "Error", code: error?.code || "UNHANDLED_REQUEST" });
    if (response.headersSent || response.destroyed) return response.destroy();
    const status = error?.status || (error instanceof SyntaxError ? 400 : 500);
    return sendJson(response, status, { error: status === 413 ? "Payload excede o limite permitido." : status < 500 ? "Solicitação inválida." : "Erro interno." }, status === 413 ? { Connection: "close" } : {});
  });
});
...
server.listen(config.port, config.host, () => console.log(`Bot da ${config.storeName} ativo em http://${config.host}:${config.port}`));
