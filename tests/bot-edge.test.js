...
test("valida produto, variação e quantidades inválidas", async () => {
  ...
  await bot.handle("1", "adicionar JAL-001");
...
test("impede estoque acumulado acima do disponível", async () => {
  const { bot } = setup();
  await add(bot, "2", "JAL-001", "M", 20);
  await bot.handle("2", "adicionar JAL-001"); await bot.handle("2", "M");
...
test("considera todas as variações no limite total de estoque", async () => {
  const { bot } = setup();
  await add(bot, "2b", "JAL-001", "M", 15);
  await bot.handle("2b", "adicionar JAL-001"); await bot.handle("2b", "G");
...
test("salva, lista e remove favoritos sem duplicar", async () => {
  const { bot, sessions } = setup();
  await bot.handle("4", "favoritar JAL-001"); await bot.handle("4", "favoritar JAL-001");
  assert.equal(sessions.read()["4"].favorites.length, 1);
  assert.match((await bot.handle("4", "favoritos")).messages[0], /Jaleco Axis/);
  assert.match((await bot.handle("4", "desfavoritar JAL-001")).messages[0], /removido/);
...
test("anexa observação ao pedido e limpa após confirmação", async () => {
  ...
  await add(bot, user, "JAL-001", "M", 1);
...
test("aplica frete e frete grátis no limite configurado", async () => {
  const first = setup({ freeShippingFrom: 1000 }); await add(first.bot, "6", "JAL-001", "M", 1);
  ...
  const second = setup(); await add(second.bot, "7", "SCR-001", "M", 1);
...
test("cancelamento preserva carrinho e status respeita o dono", async () => {
  ...
  assert.match((await bot.handle(user, "carrinho")).messages[0], /Jaleco Axis/);
